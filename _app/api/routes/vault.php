...
function memberVault(): void {
    ...
    apiSuccess([
        ...
        'participation_areas'       => !empty($member['participation_answers'])
            ...
            : [],
        'op_thread_summary'         => fetchOpThreadSummary($db, (int)($member['id'] ?? 0), !empty($member['participation_answers'])
            ? (is_string($member['participation_answers'])
                ? (json_decode($member['participation_answers'], true) ?: [])
                : (array)$member['participation_answers'])
            : []),
...
function fetchOpThreadSummary(PDO $db, int $memberId, array $areas): array {
    ...
    $out = [];
    try {
        // Unread broadcasts per area
        $ph = implode(',', array_fill(0, count($areas), '?'));
        $bStmt = $db->prepare(
            "SELECT t.area_key, COUNT(*) AS unread
             FROM partner_op_threads t
             INNER JOIN partner_op_broadcast_reads br ON br.thread_id = t.id AND br.member_id = ?
             WHERE t.direction = 'broadcast' AND t.area_key IN ({$ph}) AND br.read_at IS NULL
             GROUP BY t.area_key"
        );
        $bStmt->execute(array_merge([$memberId], $areas));
        $unreadBc = [];
        foreach ($bStmt->fetchAll() as $r) { $unreadBc[$r['area_key']] = (int)$r['unread']; }

        // Open inbound threads per area (member's own, with unread admin replies)
        $tStmt = $db->prepare(
            "SELECT t.area_key,
                    COUNT(DISTINCT t.id) AS open_threads,
                    SUM(CASE WHEN r.direction = 'outbound' AND r.read_at IS NULL THEN 1 ELSE 0 END) AS unread_replies
             FROM partner_op_threads t
             LEFT JOIN partner_op_replies r ON r.thread_id = t.id
             WHERE t.initiated_by_member_id = ? AND t.area_key IN ({$ph}) AND t.status != 'archived'
             GROUP BY t.area_key"
        );
        $tStmt->execute(array_merge([$memberId], $areas));
        $threadInfo = [];
        foreach ($tStmt->fetchAll() as $r) {
            $threadInfo[$r['area_key']] = [
                'open_threads'   => (int)$r['open_threads'],
                'unread_replies' => (int)$r['unread_replies'],
            ];
        }
        ...
        foreach ($areas as $areaKey) {
            $out[] = [
                'area_key'       => $areaKey,
                'unread_bc'      => (int)($unreadBc[$areaKey] ?? 0),
                'open_threads'   => (int)($threadInfo[$areaKey]['open_threads'] ?? 0),
                'unread_replies' => (int)($threadInfo[$areaKey]['unread_replies'] ?? 0),
            ];
        }
    } catch (Throwable) {}
    return $out;
...
function handlePartnerOpThreads(): void {
    ...
    // Resolve member id
    $subjectRef = (string)$principal['subject_ref'];
    $mStmt = $db->prepare('SELECT id, participation_answers FROM members WHERE member_type = ? AND (member_number = ? OR id = ?) LIMIT 1');
    $mStmt->execute(['personal', $subjectRef, (int)$principal['principal_id']]);
    $member = $mStmt->fetch();
    if (!$member) apiError('Member not found.', 404);
    $memberId = (int)$member['id'];
    ...
    if ($method === 'POST') {
        ...
        $db->prepare(
            "INSERT INTO partner_op_threads (area_key, direction, subject, body, status, initiated_by_member_id, created_at, updated_at)
             VALUES (?, 'inbound', ?, ?, 'open', ?, NOW(), NOW())"
        )->execute([$areaKey, $subject, $msg, $memberId]);

        apiSuccess(['created' => true, 'thread_id' => (int)$db->lastInsertId()]);
    }

    // GET — return all threads + broadcasts for enrolled areas
    if (!$areas) { apiSuccess(['areas' => []]); }

    $ph = implode(',', array_fill(0, count($areas), '?'));

    // Broadcasts delivered to this member
    $bcStmt = $db->prepare(
        "SELECT t.id, t.area_key, t.subject, t.body, t.created_at,
                br.delivered_at, br.read_at
         FROM partner_op_threads t
         INNER JOIN partner_op_broadcast_reads br ON br.thread_id = t.id AND br.member_id = ?
    ...
    // Member's own inbound threads with their replies
    $thStmt = $db->prepare(
        "SELECT id, area_key, subject, body, status, reply_count, last_reply_at, created_at
         FROM partner_op_threads
         WHERE initiated_by_member_id = ? AND area_key IN ({$ph}) AND status != 'archived'
    ...
    $replies = [];
    if ($threads) {
        $tIds = array_map(fn($r) => (int)$r['id'], $threads);
        $rph  = implode(',', array_fill(0, count($tIds), '?'));
        $rStmt = $db->prepare(
            "SELECT thread_id, body, direction, created_at FROM partner_op_replies
             WHERE thread_id IN ({$rph}) ORDER BY created_at ASC"
        );
        $rStmt->execute($tIds);
        foreach ($rStmt->fetchAll() as $r) {
            $replies[(int)$r['thread_id']][] = $r;
        }
        // Mark outbound replies as read
        $db->prepare(
            "UPDATE partner_op_replies SET read_at = NOW()
             WHERE thread_id IN ({$rph}) AND direction = 'outbound' AND read_at IS NULL"
        )->execute($tIds);
    }
    ...
    foreach ($threads as $th) {
        ...
        if (isset($byArea[$ak])) {
            $th['replies'] = $replies[(int)$th['id']] ?? [];
            $byArea[$ak]['threads'][] = $th;
        }
    }
...
function handlePartnerOpReply(): void {
    ...
    // Resolve member
    $subjectRef = (string)$principal['subject_ref'];
    $mStmt = $db->prepare('SELECT id FROM members WHERE member_type = ? AND (member_number = ? OR id = ?) LIMIT 1');
    $mStmt->execute(['personal', $subjectRef, (int)$principal['principal_id']]);
    $member = $mStmt->fetch();
    if (!$member) apiError('Member not found.', 404);
    $memberId = (int)$member['id'];
    ...
    $tStmt = $db->prepare('SELECT id, status FROM partner_op_threads WHERE id = ? AND initiated_by_member_id = ? LIMIT 1');
    $tStmt->execute([$threadId, $memberId]);
    $thread = $tStmt->fetch();
    if (!$thread) apiError('Thread not found.', 404);
    if ((string)$thread['status'] === 'closed') apiError('This thread has been closed.');

    $db->prepare(
        "INSERT INTO partner_op_replies (thread_id, body, direction, from_member_id, created_at) VALUES (?, ?, 'inbound', ?, NOW())"
    )->execute([$threadId, $msg, $memberId]);

    $db->prepare(
        "UPDATE partner_op_threads SET reply_count = reply_count + 1, last_reply_at = NOW(), status = 'open', updated_at = NOW() WHERE id = ?"
    )->execute([$threadId]);
...
function handlePartnerOpRead(): void {
    ...
    $db->prepare(
        "UPDATE partner_op_broadcast_reads SET read_at = NOW() WHERE thread_id = ? AND member_id = ? AND read_at IS NULL"
