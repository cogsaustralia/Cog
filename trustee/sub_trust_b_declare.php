...
/**
 * trustee/sub_trust_b_declare.php
...
// ── Token gate: accept both unused (Step A) and used (Step B) tokens
// The token is consumed on Step A POST. Step B uses the same URL.
// If unused  → first visit, Step A pending.
// If used    → Step B in progress; load active session by deed_key.
$tokenHash = hash('sha256', $rawToken);
$stmtToken = $db->prepare(
    'SELECT id, used_at FROM one_time_tokens
     WHERE token_hash = ? AND purpose = \'sub_trust_b_execution\'
       AND expires_at > UTC_TIMESTAMP()
     LIMIT 1'
);
$stmtToken->execute([$tokenHash]);
$tokenRow = $stmtToken->fetch(PDO::FETCH_ASSOC);
$tokenIsUsed = $tokenRow && $tokenRow['used_at'] !== null;
if (!$tokenRow) de_abort(403, 'Token is invalid or expired.');

// ── Session ID: from URL param, or recovered from DB for Step B
$sessionId = trim((string)($_GET['session'] ?? ''));

// ── Active session from DB
$activeSession = null;
if ($sessionId !== '') {
    $activeSession = SubTrustBExecutionService::getSession($db, $sessionId);
}
// If token already used but no session in URL, recover via deed_key (Step B path)
if ($tokenIsUsed && !$activeSession) {
    $activeSession = SubTrustBExecutionService::getActiveSession($db);
    if ($activeSession) {
        $sessionId = $activeSession['session_id'];
    } else {
        de_abort(403, 'Token already used and no active execution session found.');
    }
}
...
$deedSha256 = SubTrustBExecutionService::getDeedSha256($db)
    ?? SubTrustBExecutionService::DEED_SHA256;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($postedToken !== $rawToken) {
        ...
        if (!$tokenIsUsed && !SubTrustBExecutionService::validateOneTimeToken($db, $rawToken, 'sub_trust_b_execution')) {
            de_abort(403, 'Token is no longer valid.');
        }

        try {
            $postResult = SubTrustBExecutionService::recordExecution(
                $db, $capacity, $postedSid, $postedHash,
                getClientIp(), (string)($_SERVER['HTTP_USER_AGENT'] ?? '')
            );
            if (!$tokenIsUsed) {
                SubTrustBExecutionService::consumeOneTimeToken($db, $rawToken, 'sub_trust_b_execution');
                $tokenIsUsed = true;
            }
            $sessionId = $postedSid;
            $activeSession = SubTrustBExecutionService::getSession($db, $sessionId);
            ...
            $deedSha256 = SubTrustBExecutionService::getDeedSha256($db) ?? SubTrustBExecutionService::DEED_SHA256;
            $deedSha256Display = $deedSha256;
        } catch (\Throwable $e) {
...
<div class="crest">
  ...
  <h1>Sub-Trust B Execution — Deed</h1>
...
<div class="done-card">
  ...
  <div class="notice">
    ...
    <strong style="color:var(--gold)"><?= de_h(SubTrustBExecutionService::WITNESS_NAME) ?></strong>
    out-of-band. The witness must complete their attestation before this deed is fully executed.
  </div>
  <div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:20px">
    <a href="../docs/<?= de_h(SubTrustBExecutionService::DEED_PDF) ?>"
    ...
    <a href="../admin/execution_records.php?cert=sub_trust_b"
       style="display:inline-block;padding:10px 18px;border-radius:8px;
...
<div class="card">
  <div class="card-title">Declaration — <?= de_h(SubTrustBExecutionService::DEED_VERSION) ?></div>
  <div class="hash-box">
    <span class="lbl">Deed PDF SHA-256 (<?= de_h(SubTrustBExecutionService::DEED_VERSION) ?>)</span>
    <?= de_h($deedSha256Display) ?>
  </div>
  <a class="dl" href="../docs/<?= de_h(SubTrustBExecutionService::DEED_PDF) ?>" target="_blank" rel="noopener">
...
$ackText     = $currentCap === 'declarant'
    ? SubTrustBExecutionService::DECLARANT_ACKNOWLEDGEMENT
    : SubTrustBExecutionService::TRUSTEE_ACKNOWLEDGEMENT;
...
<form method="POST" action="<?= de_h($pageUrl) ?>" id="execForm">
  ...
  <div class="card step-active">
    ...
    <div class="cb-row">
      ...
      <label for="cb_read">
        I have read the full <strong><?= de_h(SubTrustBExecutionService::DEED_TITLE) ?></strong>
        (<?= de_h(SubTrustBExecutionService::DEED_VERSION) ?>) and understand its terms.
      </label>
    </div>
