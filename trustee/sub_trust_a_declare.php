...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($postedToken !== $rawToken) {
        ...
        if (!$tokenIsUsed && !SubTrustAExecutionService::validateOneTimeToken($db, $rawToken, 'sub_trust_a_execution')) {
        ...
        try {
            ...
            if (!$tokenIsUsed) {
                SubTrustAExecutionService::consumeOneTimeToken($db, $rawToken, 'sub_trust_a_execution');
                $tokenIsUsed = true;
            }
            $sessionId = $postedSid;
            $activeSession = SubTrustAExecutionService::getSession($db, $sessionId);
...
<div class="done-card">
  ...
  <div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:20px">
    <a href="../docs/<?= de_h(SubTrustAExecutionService::DEED_PDF) ?>"
