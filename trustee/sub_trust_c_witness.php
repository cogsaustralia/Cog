...
/**
 * trustee/sub_trust_c_witness.php
...
$stmt = $db->prepare(
    'SELECT id FROM one_time_tokens
     WHERE token_hash = ? AND purpose = \'sub_trust_c_witness\'
...
$session = SubTrustCExecutionService::getSession($db, $sessionId);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($postedToken !== $rawToken || $postedSession !== $sessionId) {
        ...
        if (!SubTrustCExecutionService::validateOneTimeToken($db, $rawToken, 'sub_trust_c_witness')) {
            wa_abort(403, 'Token is no longer valid.');
        }
        try {
            $result = SubTrustCExecutionService::recordWitnessAttestation(
...
<title>Sub-Trust C Witness Attestation — COG$ Foundation</title>
<style>
...
<div class="result-card">
  ...
  <div class="notice">
    ...
    Electronic Transactions Act 2000 (NSW). The <?= wa_h(SubTrustCExecutionService::DEED_TITLE) ?>
    is now fully executed. This record is permanent and cryptographically anchored.
    No further action is required from you.
  </div>
  <div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:20px">
    <a href="../docs/<?= wa_h(SubTrustCExecutionService::DEED_PDF) ?>"
...
<div class="card">
  ...
  <div class="hash-box">
    <span class="lbl">Deed: <?= wa_h(SubTrustCExecutionService::DEED_TITLE) ?> — <?= wa_h(SubTrustCExecutionService::DEED_VERSION) ?></span>
    SHA-256: <?= wa_h($deedSha256) ?>
  </div>
  <a class="dl" href="../docs/<?= wa_h(SubTrustCExecutionService::DEED_PDF) ?>" target="_blank" rel="noopener">
  ...
  <div class="card">
    ...
    <div class="cb-row">
      ...
      <label for="cb_observed">
        ...
        <strong><?= wa_h(SubTrustCExecutionService::DEED_TITLE) ?></strong>
        electronically via audio-visual link on
        <strong><?= wa_h(SubTrustCExecutionService::EXECUTION_DATE) ?></strong>.
      </label>
    </div>
