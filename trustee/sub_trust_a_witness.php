...
/**
 * trustee/sub_trust_a_witness.php
...
$stmt = $db->prepare(
    'SELECT id FROM one_time_tokens
     WHERE token_hash = ? AND purpose = \'sub_trust_a_witness\'
...
$session = SubTrustAExecutionService::getSession($db, $sessionId);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($postedToken !== $rawToken || $postedSession !== $sessionId) {
        ...
        if (!SubTrustAExecutionService::validateOneTimeToken($db, $rawToken, 'sub_trust_a_witness')) {
            wa_abort(403, 'Token is no longer valid.');
        }
        try {
            $result = SubTrustAExecutionService::recordWitnessAttestation(
                $db, $sessionId, $deedSha256,
                getClientIp(), (string)($_SERVER['HTTP_USER_AGENT'] ?? '')
            );
            SubTrustAExecutionService::consumeOneTimeToken($db, $rawToken, 'sub_trust_a_witness');
        } catch (\Throwable $e) {
...
<title>Sub-Trust A Witness Attestation — COG$ Foundation</title>
<style>
...
<div class="result-card">
  ...
  <div class="notice">
    ...
    Electronic Transactions Act 2000 (NSW). The <?= wa_h(SubTrustAExecutionService::DEED_TITLE) ?>
    is now fully executed. This record is permanent and cryptographically anchored.
    No further action is required from you.
  </div>
  <div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:20px">
    <a href="../docs/<?= wa_h(SubTrustAExecutionService::DEED_PDF) ?>"
    ...
    <a href="../admin/execution_records.php?cert=sub_trust_a"
       style="display:inline-block;padding:10px 18px;border-radius:8px;
...
<div class="card">
  ...
  <div class="hash-box">
    <span class="lbl">Deed: <?= wa_h(SubTrustAExecutionService::DEED_TITLE) ?> — <?= wa_h(SubTrustAExecutionService::DEED_VERSION) ?></span>
    SHA-256: <?= wa_h($deedSha256) ?>
  </div>
  <a class="dl" href="../docs/<?= wa_h(SubTrustAExecutionService::DEED_PDF) ?>" target="_blank" rel="noopener">
  ...
  <div class="card">
    ...
    <div class="cb-row">
      ...
      <label for="cb_observed">
        ...
        <strong><?= wa_h(SubTrustAExecutionService::DEED_TITLE) ?></strong>
        electronically via audio-visual link on
        <strong><?= wa_h(SubTrustAExecutionService::EXECUTION_DATE) ?></strong>.
      </label>
    </div>
