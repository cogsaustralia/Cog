...
$tokenError = null;
$activeSession = SubTrustAExecutionService::getActiveSession($pdo);
...
try {
    $s = $pdo->prepare(
        'SELECT purpose, expires_at, created_at FROM one_time_tokens
         WHERE purpose IN (\'sub_trust_a_execution\',\'sub_trust_a_witness\')
           AND used_at IS NULL AND expires_at > UTC_TIMESTAMP()
         ORDER BY created_at DESC'
    );
    $s->execute();
    foreach ($s->fetchAll(\PDO::FETCH_ASSOC) as $row) {
        if ($row['purpose'] === 'sub_trust_a_execution' && !$execToken)    $execToken    = $row;
        if ($row['purpose'] === 'sub_trust_a_witness'  && !$witnessToken)  $witnessToken = $row;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $canGenerate) {
    ...
    if ($action === 'generate_exec_token') {
        try {
            $rawToken = SubTrustAExecutionService::generateOneTimeToken($pdo, 'sub_trust_a_execution');
            // Re-fetch
            $s = $pdo->prepare('SELECT purpose,expires_at,created_at FROM one_time_tokens WHERE purpose=\'sub_trust_a_execution\' AND used_at IS NULL AND expires_at>UTC_TIMESTAMP() LIMIT 1');
        ...
        if (!$bothDone) {
            ...
            try {
                $rawWitnessToken = SubTrustAExecutionService::generateOneTimeToken($pdo, 'sub_trust_a_witness');
                $s = $pdo->prepare('SELECT purpose,expires_at,created_at FROM one_time_tokens WHERE purpose=\'sub_trust_a_witness\' AND used_at IS NULL AND expires_at>UTC_TIMESTAMP() LIMIT 1');
                $s->execute(); $witnessToken = $s->fetch(\PDO::FETCH_ASSOC) ?: null;
            } catch (\Throwable $e) { $tokenError = $e->getMessage(); }
        }

    } elseif ($action === 'invalidate_exec_token') {
        $pdo->prepare('UPDATE one_time_tokens SET used_at=UTC_TIMESTAMP() WHERE purpose=\'sub_trust_a_execution\' AND used_at IS NULL')->execute();
        header('Location: ./generate_sub_trust_a_token.php?flash=Token+invalidated&type=ok'); exit;

    } elseif ($action === 'invalidate_witness_token') {
        $pdo->prepare('UPDATE one_time_tokens SET used_at=UTC_TIMESTAMP() WHERE purpose=\'sub_trust_a_witness\' AND used_at IS NULL')->execute();
        header('Location: ./generate_sub_trust_a_token.php?flash=Witness+token+invalidated&type=ok'); exit;
    }
}
...
<title>Sub-Trust A Tokens | COG$ Admin</title>
<?php if (function_exists('ops_admin_help_assets_once')) ops_admin_help_assets_once(); ?>
</head>
<body>
<div class="admin-shell">
<?php admin_sidebar_render('generate_sub_trust_a_token'); ?>
<style>
...
<div class="main">
  <div class="topbar">
    <h2>📜 Sub-Trust A Execution Tokens</h2>
    <p><?= gdt_h(SubTrustAExecutionService::DEED_TITLE) ?> — <?= gdt_h(SubTrustAExecutionService::DEED_VERSION) ?><br>
  ...
  <?php if ($fullyExecuted): ?>
    <div class="banner-ok">✓ Sub-Trust A Deed fully executed — both capacities complete and witness attestation recorded.</div>
  <?php elseif ($bothDone): ?>
  ...
  <?php else: ?>
    <div class="banner-err">⚠ Sub-Trust A Deed not yet executed.</div>
  <?php endif; ?>
  ...
  <div class="card" style="border-color:rgba(82,184,122,.35)">
    ...
    <div style="display:flex;gap:10px;flex-wrap:wrap;padding:4px 0">
      <a href="<?= gdt_h(admin_url('../docs/' . SubTrustAExecutionService::DEED_PDF)) ?>"
         target="_blank" rel="noopener" class="btn btn-gold">
        ↓ Download Deed PDF
      </a>
      <a href="<?= gdt_h(admin_url('execution_records.php')) ?>?cert=sub_trust_a"
         class="btn">
  ...
  <div class="card">
    ...
    <?php elseif (!$execToken && !$rawToken && !$activeSession): ?>
      <p style="font-size:.85rem;color:var(--sub);margin:0 0 14px">Generate a one-time token for Thomas to access the Sub-Trust A Deed execution flow.</p>
      <form method="POST">
    ...
    <?php if ($rawToken): ?>
      ...
      <div class="url-box"><a href="<?= gdt_h($host) ?>/trustee/sub_trust_a_declare.php?token=<?= gdt_h($rawToken) ?>" target="_blank" rel="noopener" style="color:var(--gold);word-break:break-all"><?= gdt_h($host) ?>/trustee/sub_trust_a_declare.php?token=<?= gdt_h($rawToken) ?></a></div>
    <?php endif; ?>
  </div>

  <!-- Witness token (Alex) — only available after both capacities done -->
  <div class="card" style="<?= $bothDone ? '' : 'opacity:.45;pointer-events:none' ?>">
    <div class="card-title">Step 2 — Witness Token (<?= gdt_h(SubTrustAExecutionService::WITNESS_NAME) ?>)</div>
    <?php if (!$bothDone): ?>
    ...
    <?php elseif (!$witnessToken && !$rawWitnessToken): ?>
      <p style="font-size:.85rem;color:var(--sub);margin:0 0 14px">Generate a one-time witness token for <?= gdt_h(SubTrustAExecutionService::WITNESS_NAME) ?>. Deliver out-of-band.</p>
      <form method="POST">
    ...
    <?php if ($rawWitnessToken && $sessionId): ?>
      ...
      <div class="url-box"><a href="<?= gdt_h($host) ?>/trustee/sub_trust_a_witness.php?token=<?= gdt_h($rawWitnessToken) ?>&session=<?= gdt_h($sessionId) ?>" target="_blank" rel="noopener" style="color:var(--gold);word-break:break-all"><?= gdt_h($host) ?>/trustee/sub_trust_a_witness.php?token=<?= gdt_h($rawWitnessToken) ?>&session=<?= gdt_h($sessionId) ?></a></div>
    <?php endif; ?>
  </div>
