...
$tokenError = null;
$activeSession = SubTrustBExecutionService::getActiveSession($pdo);
...
try {
    ...
    foreach ($s->fetchAll(\PDO::FETCH_ASSOC) as $row) {
        if ($row['purpose'] === 'sub_trust_b_execution' && !$execToken)    $execToken    = $row;
        if ($row['purpose'] === 'sub_trust_b_witness'  && !$witnessToken)  $witnessToken = $row;
    }
...
<title>Sub-Trust B Tokens | COG$ Admin</title>
<?php if (function_exists('ops_admin_help_assets_once')) ops_admin_help_assets_once(); ?>
</head>
<body>
<div class="admin-shell">
<?php admin_sidebar_render('generate_sub_trust_b_token'); ?>
<style>
...
<div class="main">
  <div class="topbar">
    <h2>📜 Sub-Trust B Execution Tokens</h2>
    <p><?= gdt_h(SubTrustBExecutionService::DEED_TITLE) ?> — <?= gdt_h(SubTrustBExecutionService::DEED_VERSION) ?><br>
  ...
  <div class="card">
    ...
    <?php if ($rawToken): ?>
      ...
      <div class="url-box"><a href="<?= gdt_h($host) ?>/trustee/sub_trust_b_declare.php?token=<?= gdt_h($rawToken) ?>" target="_blank" rel="noopener" style="color:var(--gold);word-break:break-all"><?= gdt_h($host) ?>/trustee/sub_trust_b_declare.php?token=<?= gdt_h($rawToken) ?></a></div>
    <?php endif; ?>
  </div>

  <!-- Witness token (Alex) — only available after both capacities done -->
  <div class="card" style="<?= $bothDone ? '' : 'opacity:.45;pointer-events:none' ?>">
    <div class="card-title">Step 2 — Witness Token (<?= gdt_h(SubTrustBExecutionService::WITNESS_NAME) ?>)</div>
    <?php if (!$bothDone): ?>
    ...
    <?php elseif (!$witnessToken && !$rawWitnessToken): ?>
      <p style="font-size:.85rem;color:var(--sub);margin:0 0 14px">Generate a one-time witness token for <?= gdt_h(SubTrustBExecutionService::WITNESS_NAME) ?>. Deliver out-of-band.</p>
      <form method="POST">
    ...
    <?php if ($rawWitnessToken && $sessionId): ?>
      ...
      <div class="url-box"><a href="<?= gdt_h($host) ?>/trustee/sub_trust_b_witness.php?token=<?= gdt_h($rawWitnessToken) ?>&session=<?= gdt_h($sessionId) ?>" target="_blank" rel="noopener" style="color:var(--gold);word-break:break-all"><?= gdt_h($host) ?>/trustee/sub_trust_b_witness.php?token=<?= gdt_h($rawWitnessToken) ?>&session=<?= gdt_h($sessionId) ?></a></div>
    <?php endif; ?>
  </div>
