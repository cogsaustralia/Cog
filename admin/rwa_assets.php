...
<div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:14px">
    ...
    <div class="card"><div class="card-head"><h2>COG$ available <?= ops_admin_help_button('COG$ available', 'Calculated from the latest valuation basis using the Stage 1 live backing views at $4 of value per RWA COG$.') ?></h2></div><div class="card-body"><div style="font-size:1.8rem;font-weight:800"><?= rwa_num($summary['available']) ?></div></div></div>
  </div>
