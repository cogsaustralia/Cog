...
function ex_update_batch_request_state(PDO $pdo, int $batchId, string $requestStatus, string $itemStatus): void {
    ...
    $pdo->prepare("UPDATE execution_requests er JOIN execution_batch_items ebi ON ebi.execution_request_id = er.id SET er.execution_status = ?, er.updated_at = NOW() WHERE ebi.execution_batch_id = ?")->execute([$requestStatus, $batchId]);
}
        ...
        try {
            if ($action === 'create_request_from_approval') {
                ex_in_tx($pdo, function () use ($pdo, $adminUserId, &$flash): void {
                    ...
                    if ($existing) { $flash = 'Execution request already exists for this approval.'; return; }
                    ...
                    $stmt->execute([ex_req_key($approvalId), $approvalId, $approval['mint_queue_id'] ? (int)$approval['mint_queue_id'] : null, $partnerId ?: null, (int)$approval['member_id'], (int)$approval['token_class_id'], $units > 0 ? $units : null, null, $approval['evidence_reference'] ?: null, $adminUserId, $adminUserId]);
                    ...
                    if (!empty($backingAttach['required'])) {
                        ...
                        if (!empty($backingAllocIds) && ops_has_table($pdo, 'stewardship_backing_allocations')) {
                            ...
                            $tradeIds = array_filter(array_column(ops_fetch_all($pdo, "SELECT asx_trade_id FROM stewardship_backing_allocations WHERE id IN ($placeholders) AND asx_trade_id IS NOT NULL", $backingAllocIds), 'asx_trade_id'));
                        }
                        $docHashes = (!empty($tradeIds) && function_exists('ops_get_asx_trade_document_hashes')) ? ops_get_asx_trade_document_hashes($pdo, array_map('intval', $tradeIds)) : [];
                        $payload = ['asset_backing_allocation_ids' => $backingAllocIds];
                        if (!empty($docHashes)) { $payload['trade_document_hashes'] = $docHashes; }
                        $pdo->prepare("UPDATE execution_requests SET payload_json = ?, updated_at = NOW() WHERE id = ?")->execute([json_encode($payload, JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE), $executionRequestId]);
                    }
                    if (!empty($approval['mint_queue_id'])) {
                        $pdo->prepare("UPDATE mint_queue SET execution_request_id = ?, updated_at = NOW() WHERE id = ?")->execute([$executionRequestId, (int)$approval['mint_queue_id']]);
                    }
                ...
                ex_in_tx($pdo, function () use ($pdo, $adminUserId, $legacyAdminId, &$flash): void {
                    ...
                    $legacyBatchId = null;
                    $hasLegacy = false;
                    foreach ($selected as $s) { if (!empty($s['legacy_mint_queue_id'])) { $hasLegacy = true; break; } }
                    if ($hasLegacy) {
                        ...
                        $stmt = $pdo->prepare("INSERT INTO mint_batches (batch_code, batch_label, chain_target, ledger_target, network_ref, shard_ref, batch_status, created_by_admin_id, reviewed_by_admin_id, notes, created_at, updated_at) VALUES (?, ?, 'besu-prep', 'phase1-parallel', 'phase1-parallel', 'registry-main', 'prepared', ?, ?, ?, NOW(), NOW())");
                    ...
                    $stmt = $pdo->prepare("INSERT INTO execution_batches (batch_key, legacy_mint_batch_id, ledger_target, batch_status, created_by_admin_user_id, reviewed_by_admin_user_id, notes, created_at, updated_at) VALUES (?, ?, 'phase1-parallel', 'prepared', ?, ?, ?, NOW(), NOW())");
                    ...
                    foreach ($selected as $s) {
                        ...
                        if ($legacyBatchId && $insertLegacyItem && $updateMintQueue && !empty($s['legacy_mint_queue_id'])) {
                            ...
                            $insertLegacyItem->execute([$legacyBatchId, (int)$mq['id'], (int)$mq['member_id'], (int)$mq['token_class_id'], (int)$mq['approval_request_id'], (string)$mq['queue_status'], (string)($mq['manual_signoff_lane'] ?? ''), $mq['notes'] ?? null, $mq['evidence_reference'] ?? null]);
                ...
                ex_in_tx($pdo, function () use ($pdo, &$flash): void {
                    ...
                    $pdo->prepare("INSERT INTO quorum_requests (request_key, execution_batch_id, required_signatures, status, opened_at, notes) VALUES (?, ?, 3, 'open', NOW(), ?)")->execute([ex_quorum_key($batchId), $batchId, 'Opened from execution console']);
                ...
                ex_in_tx($pdo, function () use ($pdo, $legacyAdminId, &$flash): void {
                    ...
                    if ($submission) {
                        $pdo->prepare("UPDATE execution_submissions SET ledger_tx_hash = ?, submission_status='submitted', submitted_at = COALESCE(submitted_at, NOW()), notes = ? WHERE id = ?")->execute([$txHash !== '' ? $txHash : null, 'Submitted from execution console', (int)$submission['id']]);
                    } else {
                        $pdo->prepare("INSERT INTO execution_submissions (execution_batch_id, ledger_tx_hash, submission_status, submitted_at, notes) VALUES (?, ?, 'submitted', NOW(), ?)")->execute([$batchId, $txHash !== '' ? $txHash : null, 'Submitted from execution console']);
                    }
                    ...
                    if (!empty($batch['legacy_mint_batch_id'])) {
                        ...
                        if ($handoff) {
                            $pdo->prepare("UPDATE chain_handoffs SET handoff_status='submitted', submission_status='submitted', tx_reference = COALESCE(?, tx_reference), updated_at = NOW() WHERE id = ?")->execute([$txHash !== '' ? $txHash : null, (int)$handoff['id']]);
                        } else {
                            $pdo->prepare("INSERT INTO chain_handoffs (mint_batch_id, handoff_code, chain_target, ledger_target, network_ref, shard_ref, handoff_status, submission_status, tx_reference, prepared_by_admin_id, reviewed_by_admin_id, notes, created_at, updated_at) VALUES (?, ?, 'besu-prep', 'phase1-parallel', 'phase1-parallel', 'registry-main', 'submitted', 'submitted', ?, ?, ?, ?, NOW(), NOW())")->execute([(int)$batch['legacy_mint_batch_id'], ex_handoff_code((int)$batch['legacy_mint_batch_id']), $txHash !== '' ? $txHash : null, $legacyAdminId, $legacyAdminId, 'Submitted from execution console']);
                        }
                    }
                ...
                ex_in_tx($pdo, function () use ($pdo, &$flash): void {
                    ...
                    if (!empty($batch['legacy_mint_batch_id'])) {
                        ex_assert(ops_has_table($pdo, 'chain_handoffs'), 'Legacy bridge table chain_handoffs is unavailable.');
                        $pdo->prepare("UPDATE chain_handoffs SET handoff_status='finalised', submission_status='finalised', finalised_at = NOW(), updated_at = NOW() WHERE mint_batch_id = ?")->execute([(int)$batch['legacy_mint_batch_id']]);
                    }
                ...
                ex_in_tx($pdo, function () use ($pdo, $adminUserId, &$flash): void {
                    ...
                    foreach ($reqs as $r) {
                        ...
                        if (!$hasPub->fetchColumn()) { $pubStmt->execute([(int)$r['id'], 'PUB-' . (int)$r['id'] . '-' . date('YmdHis')]); }
                        if (!empty($r['legacy_mint_queue_id']) && $updateLegacyQueue) { $updateLegacyQueue->execute([(int)$r['legacy_mint_queue_id']]); }
...
<div class="card">
  <div class="card-body">
    <h2>Execution console <?= ops_admin_help_button('Execution console', 'Use this page for live execution operations only. The normal operator flow is: create request, batch, open quorum, mark quorum met, mark submitted, mark finalised, then publish.') ?></h2>
    <p class="muted">Authoritative execution surface for batching, quorum, submission, finality, and publication.</p>
  </div>
</div>
...
<div class="stat-grid" style="margin-bottom:18px">
  <?php foreach ($counts as $label => $val): ?>
    <div class="card">
      <div class="card-body">
        <div class="stat-label"><?= ops_h($label) ?></div>
...
<div class="card">
  <div class="card-head">
    <h2>Approvals ready <?= ops_admin_help_button('Approvals ready', 'These rows are eligible to become execution requests. Creating an execution request does not batch, submit, or publish anything.') ?></h2>
    <span class="muted small">approval_requests → execution_requests</span>
  </div>
  <div class="card-body table-wrap"><table>
    <thead><tr>
      <th>Approval</th>
      <th><?= ops_h($partnerLabel) ?></th>
      <th>Member #</th>
      <th>Class <?= ops_admin_help_button('Class', 'The token/class being moved into the execution workflow.') ?></th>
      <th>Units <?= ops_admin_help_button('Units', 'The quantity approved for this execution item.') ?></th>
      <th>Backing</th>
      <th>Action <?= ops_admin_help_button('Create execution request', 'Creates the request-level execution record. Does not create a batch or change live wallet state.') ?></th>
    </tr></thead>
    <tbody>
      <?php if (!$readyApprovals): ?><tr><td colspan="7" class="empty">No approved items waiting for execution request creation.</td></tr><?php endif; ?>
      <?php foreach ($readyApprovals as $row): ?>
      <tr>
        <td>#<?= (int)$row['id'] ?></td>
        <td><?= ops_h($row['member_name'] ?? '') ?></td>
        <td class="mono"><?= ops_h($row['member_number'] ?? '') ?></td>
        ...
        <td>
          ...
          <?php if (!empty($backing['required'])): ?>
            <span class="st <?= !empty($backing['is_fully_backed']) ? 'st-ok' : 'st-warn' ?>"><?= ops_h(!empty($backing['is_fully_backed']) ? 'Backed' : 'Awaiting') ?></span>
            <div class="muted-note" style="margin-top:4px"><?= number_format((float)($backing['remaining_units'] ?? 0), 4) ?> rem · <a href="./asset_backing.php#approval-<?= (int)$row['id'] ?>">Open</a></div>
          <?php else: ?>
            <span class="muted small">Not required</span>
          <?php endif; ?>
        </td>
        <td>
          <?php if ($canManage): ?>
          <form method="post" class="mini-form">
            ...
            <button class="small-btn" type="submit">Create request</button>
          </form>
          <?php else: ?><span class="muted small">Read only</span><?php endif; ?>
        </td>
      </tr>
      <?php endforeach; ?>
    </tbody>
...
<div class="card">
  <div class="card-head">
    <h2>Execution requests <?= ops_admin_help_button('Execution requests', 'The request-level queue. Select compatible requests and batch them together.') ?></h2>
    <span class="muted small">v_phase1_execution_console</span>
  </div>
  <form method="post">
    <input type="hidden" name="_csrf" value="<?= ops_h($csrf) ?>">
    ...
    <div class="table-wrap"><table>
      <thead><tr>
        ...
        <th>Request <?= ops_admin_help_button('Request key', 'The unique execution request reference.') ?></th>
        <th><?= ops_h($partnerLabel) ?></th>
        <th>Class</th>
        <th>Status <?= ops_admin_help_button('Status', 'Shows where the request currently sits.') ?></th>
        <th>Batch <?= ops_admin_help_button('Batch', 'The governed processing bundle.') ?></th>
        <th>Submission</th>
        <th>Publication</th>
        <th>Failure</th>
      </tr></thead>
      <tbody>
        <?php if (!$rows): ?><tr><td colspan="9" class="empty">No execution rows found.</td></tr><?php endif; ?>
        ...
        <tr>
          ...
          <td><span class="st st-dim"><?= ops_h($row['execution_status'] ?? '') ?></span></td>
          <td class="mono small"><?= ops_h($row['batch_key'] ?? '—') ?></td>
    ...
    <div class="card-body actions">
      <button type="submit" class="btn btn-gold">Batch selected requests</button>
      <span class="muted-note">Select approved/reviewed requests not already batched. <?= ops_admin_help_button('Batch selected', 'Groups selected execution requests into one operator bundle.') ?></span>
    </div>
    <?php endif; ?>
  </form>
</div>
...
<div class="card">
  <div class="card-head">
    <h2>Execution batches <?= ops_admin_help_button('Execution batches', 'Live control area. All major operator actions happen here: open quorum, mark quorum met, mark submitted, mark finalised, and publish.') ?></h2>
    <span class="muted small">Operational controls</span>
  </div>
  <div class="table-wrap"><table>
    <thead><tr>
      <th>Batch <?= ops_admin_help_button('Batch key', 'Unique reference for the governed processing bundle.') ?></th>
      <th>Status <?= ops_admin_help_button('Batch status', "The batch's current stage in the operator lifecycle.") ?></th>
      <th>Items</th>
      <th>Quorum <?= ops_admin_help_button('Quorum', 'Open quorum starts the formal sign-off stage.') ?></th>
      <th>Submission <?= ops_admin_help_button('Submission', 'Whether the batch has been formally recorded as submitted.') ?></th>
      <th>Actions <?= ops_admin_help_button('Batch actions', 'Use these in order. Stage-gated so you cannot submit, finalise, or publish out of order.') ?></th>
    </tr></thead>
    <tbody>
      <?php if (!$batchRows): ?><tr><td colspan="6" class="empty">No execution batches yet.</td></tr><?php endif; ?>
      <?php foreach ($batchRows as $row): ?>
      <tr>
        <td class="mono small"><?= ops_h($row['batch_key'] ?? '') ?></td>
        <td><span class="st st-dim"><?= ops_h($row['batch_status'] ?? '') ?></span></td>
        ...
        <td>
          <div class="actions">
            ...
            <?php if ($canManage && ex_ui_can_mark_quorum_met($row)): ?>
              <form method="post" class="mini-form"><input type="hidden" name="_csrf" value="<?= ops_h($csrf) ?>"><input type="hidden" name="action" value="mark_quorum_met"><input type="hidden" name="execution_batch_id" value="<?= (int)$row['id'] ?>"><button class="small-btn" type="submit">Mark quorum met</button></form>
            <?php endif; ?>
            <?php if ($canManage && ex_ui_can_submit($row)): ?>
              <form method="post" class="mini-form">
                ...
                <input class="mini-input" type="text" name="ledger_tx_hash" value="<?= ops_h($row['ledger_tx_hash'] ?? '') ?>" placeholder="ledger tx hash (optional)">
                <button class="small-btn" type="submit">Mark submitted</button>
              </form>
            <?php endif; ?>
            <?php if ($canManage && ex_ui_can_finalise($row)): ?>
              <form method="post" class="mini-form"><input type="hidden" name="_csrf" value="<?= ops_h($csrf) ?>"><input type="hidden" name="action" value="mark_finalised"><input type="hidden" name="execution_batch_id" value="<?= (int)$row['id'] ?>"><button class="small-btn" type="submit">Mark finalised</button></form>
            <?php endif; ?>
            <?php if ($canManage && ex_ui_can_publish($row)): ?>
              <form method="post" class="mini-form"><input type="hidden" name="_csrf" value="<?= ops_h($csrf) ?>"><input type="hidden" name="action" value="publish_batch"><input type="hidden" name="execution_batch_id" value="<?= (int)$row['id'] ?>"><button class="small-btn btn-gold" type="submit">Publish</button></form>
            <?php endif; ?>
          </div>
        </td>
      </tr>
...
<div class="card">
  <div class="card-head">
    <h2>Legacy execution bridge <?= ops_admin_help_button('Legacy execution bridge', 'Diagnostic table. Shows how the current execution path maps to older mint queue / handoff records. Use for traceability, not as the primary operator surface.') ?></h2>
    <span class="muted small">v_phase1_legacy_execution_bridge</span>
  </div>
  <div class="table-wrap"><table>
    <thead><tr><th>Mint queue</th><th>Queue status</th><th>Execution request</th><th>Execution status</th><th>Batch</th><th>Handoff</th><th>Submission</th></tr></thead>
    <tbody>
      <?php if (!$legacy): ?><tr><td colspan="7" class="empty">No legacy bridge rows found.</td></tr><?php endif; ?>
      ...
      <tr>
        ...
        <td class="mono small"><?= ops_h(($row['batch_code'] ?? '') ?: '—') ?></td>
