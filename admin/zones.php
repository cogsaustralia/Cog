...
.stat{padding:18px;border-radius:14px;background:rgba(255,255,255,.03);border:1px solid var(--line)}
.stat strong{display:block;font-size:1.8rem;font-weight:800;margin-top:6px}
...
@media(max-width:980px){.ops-grid,.form-grid,.stats{grid-template-columns:1fr}}
</style>
...
<div class="card">
  <div class="card-head"><h1 style="margin:0">Zones &amp; eligibility control plane <?= ops_admin_help_button('Zones & eligibility control plane', 'Use this page to explain why a zone exists, what approval state it is in, who is in scope, and whether any open challenge or missing evidence is preventing the zone from being treated as settled.') ?></h1></div>
  <div class="card-body" style="padding-top:6px"><p class="muted small" style="margin:0">Evidence, FNAC, board, eligibility snapshot, then challenge handling if needed.</p></div>
</div>
...
<div class="ops-grid">
  ...
  <div class="card-head"><h2>Create evidence packet<?= ops_admin_help_button('Create evidence packet', 'Use this when you need to record or refresh the evidentiary basis for a zone. This is normally the first structured operator step after a zone has been identified or changed.') ?></h2></div>
  <div class="card-body">
  ...
  <div class="card-head"><h2>Record FNAC review<?= ops_admin_help_button('Record FNAC review', 'Use this form when the First Nations Advisory Council has reviewed the zone and you need to record the review outcome on the operator ledger.') ?></h2></div>
  <div class="card-body">
  ...
  <div class="card-head"><h2>Open Board signoff<?= ops_admin_help_button('Open Board signoff', 'Open this request after the evidence and FNAC stage are ready and you need the formal governance signoff to proceed.') ?></h2></div>
  <div class="card-body">
  ...
  <div class="card-head"><h2>Create eligibility snapshot<?= ops_admin_help_button('Create eligibility snapshot', 'Use this to freeze the in-scope population for a zone at a specific point in time. This is important for later audits, disputes, and local decision votes.') ?></h2></div>
  <div class="card-body">
  ...
  <div class="card-head"><h2>Open zone challenge<?= ops_admin_help_button('Open zone challenge', 'Use this when a Partner or operator disputes the zone boundary, inclusion logic, or evidentiary basis and wants the issue tracked formally.') ?></h2></div>
  <div class="card-body">
...
<div class="card">
  <div class="card-head"><h2>Zone governance queue<?= ops_admin_help_button('Zone governance queue', 'This is the live queue view. Read this table when you need the quickest explanation of a zone: current status, FNAC state, board state, evidence count, and any ledger reference already recorded.') ?></h2></div>
...
<div class="row-grid">
  <div class="card">
  <div class="card-head"><h2>Evidence packets & FNAC reviews<?= ops_admin_help_button('Evidence packets & FNAC reviews', 'Use this trace table when someone asks: what evidence packet exists for this zone and what was the latest FNAC review outcome?') ?></h2></div>
  <div class="card-body table-wrap"><table>
  ...
  <div class="card">
  <div class="card-head"><h2>Board signoffs & snapshots<?= ops_admin_help_button('Board signoffs & snapshots', 'Use this section to confirm that governance signoff occurred and to see whether an eligibility population was frozen into a snapshot.') ?></h2></div>
  <div class="card-body table-wrap"><table>
...
<?php
// Flag members with address data but no G-NAF PID verified
...
<div class="bridge-note" style="border-color:rgba(239,68,68,.3);background:rgba(239,68,68,.06)">
  <strong style="color:var(--err)">⚠ <?= count($unverifiedAddresses) ?> member<?= count($unverifiedAddresses) !== 1 ? 's' : '' ?> with unverified G-NAF address:</strong>
  <table style="width:100%;margin-top:10px;border-collapse:collapse;font-size:.82rem">
    <thead><tr><th style="text-align:left;padding:4px 8px;color:var(--muted)">Member</th><th style="text-align:left;padding:4px 8px;color:var(--muted)">Address on file</th><th style="text-align:left;padding:4px 8px;color:var(--muted)">Issue</th></tr></thead>
    <tbody>
    <?php foreach ($unverifiedAddresses as $u): ?>
    <tr>
      <td style="padding:4px 8px"><?= ops_h($u['full_name']) ?> <span class="muted small"><?= ops_h($u['member_number']) ?></span></td>
      <td style="padding:4px 8px"><?= ops_h(trim(($u['street_address'] ?? '') . ', ' . ($u['suburb'] ?? '') . ' ' . ($u['state_code'] ?? '') . ' ' . ($u['postcode'] ?? ''))) ?></td>
      <td style="padding:4px 8px;color:var(--warn)"><?= $u['gnaf_pid'] ? 'Not verified' : 'No G-NAF PID' ?></td>
    </tr>
    <?php endforeach; ?>
    </tbody>
  </table>
  <p class="muted small" style="margin:10px 0 0">To fix: correct any typos in the member record (Members page → edit member), then re-run address verification. A common cause is a suburb name typo preventing Geoscape from matching the address.</p>
</div>
<?php endif; ?>

<div class="row-grid">
  <div class="card">
  <div class="card-head"><h2>Recent address verification activity<?= ops_admin_help_button('Recent address verification activity', 'This is the member-level input into zone eligibility. Use it to see how addresses are being matched into zones and whether verification status is blocking inclusion logic.') ?></h2></div>
  <div class="card-body table-wrap"><table>
  ...
  <div class="card">
  <div class="card-head"><h2>Zone challenges<?= ops_admin_help_button('Zone challenges', 'This section lists the open or historical disputes about zone boundaries, inclusion, or evidence. Use this when someone asks whether a zone is still contested.') ?></h2></div>
  <div class="card-body table-wrap"><table>
