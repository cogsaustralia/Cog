...
<?php if($showSummary): ?>
  ...
  <div class="card">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px"><div style="font-size:13px;font-weight:700">Recent Businesses</div><a href="./businesses.php" style="font-size:12px;color:#60d4b8;text-decoration:none">View all →</a></div>
    <div class="table-wrap"><table><thead><tr><th>Business</th><th>ABN</th><th>Payment</th><th>G-NAF</th><th>Stewardship</th><th>Status</th><th>Joined</th></tr></thead><tbody>
    <?php foreach($recentBnft as $b): $bPaid=($b['signup_payment_status']??'')=='paid'; $bGnaf=!empty($b['gnaf_pid']); $bSw=!empty($b['attestation_hash']); ?>
      <tr><td><strong style="font-size:13px"><?=h($b['legal_name']??'')?></strong><div style="font-size:11px;color:#9fb0c1"><?=h($b['email']??'')?></div></td><td style="font-family:monospace;font-size:12px;color:#9fb0c1"><?=h($b['abn']??'')?></td><td><?=$bPaid?'<span style="color:#7ee0a0;font-size:11px;font-weight:700">✓ Paid</span>':'<span style="color:#ffb4be;font-size:11px">Pending</span>'?></td><td><?=$bGnaf?'<span style="color:#60d4b8;font-size:11px">✓</span>':'<span style="color:#9fb0c1;font-size:11px">—</span>'?></td><td><?=$bSw?'<span style="color:#d4b25c;font-size:11px">✓</span>':'<span style="color:#9fb0c1;font-size:11px">—</span>'?></td><td><span style="font-size:11px;padding:2px 8px;border-radius:6px;background:rgba(255,255,255,.04);color:#9fb0c1"><?=h($b['wallet_status']??'')?></span></td><td style="font-size:11px;color:#9fb0c1"><?=h(substr($b['created_at']??'',0,10))?></td></tr>
